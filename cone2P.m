function [X, Y, Z] = cone2P(R, N,r1,r2)
    ...
    for j = 1 : m
      t=time(j);
      X(j, :) = r1x+(r2x-r1x)*t+R(j)*cos(theta)*x2x/(j^3)+R(j)*sin(theta)*x3x/(j^3); 
      Y(j, :) = r1y+(r2y-r1y)*t+R(j)*cos(theta)*x2y/(j^3)+R(j)*sin(theta)*x3y/(j^3); 
      Z(j, :) = r1z+(r2z-r1z)*t+R(j)*cos(theta)*x2z/(j^3)+R(j)*sin(theta)*x3z/(j^3);
    end
